...
class _CustomCalendarScrollViewState extends State<CustomCalendarScrollView> with TickerProviderStateMixin {
  ...
  void initState() {
    _dragDetails =
        ValueNotifier<_DragPaintDetails>(_DragPaintDetails(position: ValueNotifier<Offset?>(null)));
  ...
  Widget build(BuildContext context) {
    ...
    final bool isNeedDragAndDrop = widget.calendar.allowDragAndDrop &&
        ...
        (!widget.isMobilePlatform ||
            (widget.view != CalendarView.month && widget.view != CalendarView.timelineMonth));
    ...
    final Widget customScrollWidget = GestureDetector(
      ...
      onHorizontalDragStart: isTimelineView
          ...
          : (DragStartDetails dragStartDetails) {
              _onHorizontalStart(dragStartDetails, isResourceEnabled, isTimelineView, viewHeaderHeight,
                  timeLabelWidth, isNeedDragAndDrop);
            },
      ...
      onHorizontalDragEnd: isTimelineView
          ...
          : (DragEndDetails dragEndDetails) {
              _onHorizontalEnd(dragEndDetails, isResourceEnabled, isTimelineView, isMonthView,
                  viewHeaderHeight, timeLabelWidth, weekNumberPanelWidth, isNeedDragAndDrop);
            },
      onVerticalDragStart: isHorizontalNavigation
          ? null
          : (DragStartDetails dragStartDetails) {
              _onVerticalStart(dragStartDetails, isResourceEnabled, isTimelineView, viewHeaderHeight,
                  timeLabelWidth, isNeedDragAndDrop);
            },
                          ...
                          HorizontalDragGestureRecognizer:
                            ...
                            (HorizontalDragGestureRecognizer instance) {
                              ...
                              instance.onStart = (DragStartDetails details) {
                                _handleDragStart(details, isNeedDragAndDrop, isTimelineView,
                                    isResourceEnabled, viewHeaderHeight, timeLabelWidth, resourceViewSize);
                              };
  ...
  void _handleAppointmentDragStart(AppointmentView appointmentView, bool isTimelineView, Offset details,
    ...
    if (isResourceEnabled) {
      ...
      selectedResourceIndex =
          currentState._getSelectedResourceIndex(yPosition, viewHeaderHeight, timeLabelWidth);
    ...
    final dynamic dragStartAppointment =
        _getCalendarAppointmentToObject(appointmentView.appointment, widget.calendar);
  ...
  void _handleLongPressStart(LongPressStartDetails details, bool isNeedDragAndDrop, bool isTimelineView,
    ...
    _handleAppointmentDragStart(appointmentView, isTimelineView, details.localPosition, isResourceEnabled,
    ...
    if (widget.calendar.dragAndDropSettings.allowScroll &&
      ...
      _timer = Timer(const Duration(milliseconds: 200), () async {
        ...
        ((isHorizontalNavigation &&
                (appointmentPosition.dx + _dragDetails.value.appointmentView!.appointmentRect!.width) -
                        rtlValue >=
                    widget.width) ||
            (!isHorizontalNavigation &&
                (appointmentPosition.dy + _dragDetails.value.appointmentView!.appointmentRect!.height >=
  ...
  void _updateAppointmentDragUpdateCallback(
    ...
    if (!isMonthView) {
      if (isTimelineView) {
        ...
        draggingTime =
            DateTime(draggingTime.year, draggingTime.month, draggingTime.day, time.hour, time.minute);
      } else {
        if (yPosition < 0) {
          draggingTime = DateTime(draggingTime.year, draggingTime.month, draggingTime.day);
        } else {
          draggingTime = _timeFromPosition(draggingTime, widget.calendar.timeSlotViewSettings, yPosition,
              currentState, timeIntervalHeight, isTimelineView)!;
        }
      }
    }
    ...
    final int currentMonth =
        currentState.widget.visibleDates[currentState.widget.visibleDates.length ~/ 2].month;
  ...
  void _handleLongPressEnd(Offset details, bool isTimelineView, bool isResourceEnabled, bool isMonthView,
    ...
    final Duration appointmentDuration =
        appointment.isAllDay && widget.view != CalendarView.month && !isTimelineView
            ? const Duration(hours: 1)
    ...
    if ((appointment.recurrenceRule != null && appointment.recurrenceRule!.isNotEmpty) ||
      ...
      final List<DateTime> recurrenceDates = RecurrenceHelper.getRecurrenceDateTimeCollection(
          ...
          recurrenceDuration: AppointmentHelper.getDifference(
              parentAppointment.exactStartTime, parentAppointment.exactEndTime),
      ...
      if (appointment.recurrenceId != null &&
        ...
        widget.calendar.dataSource!
            .notifyListeners(CalendarDataSourceAction.remove, <dynamic>[appointment.data]);
      } else {
        widget.calendar.dataSource!.appointments!.remove(parentAppointment.data);
        widget.calendar.dataSource!
            .notifyListeners(CalendarDataSourceAction.remove, <dynamic>[parentAppointment.data]);
        ...
        widget.calendar.dataSource!
            .notifyListeners(CalendarDataSourceAction.add, <dynamic>[newParentAppointment]);
      }
    } else {
      widget.calendar.dataSource!.appointments!.remove(appointment.data);
      widget.calendar.dataSource!
    ...
    if (widget.calendar.onDragEnd != null) {
      widget.calendar.onDragEnd!(
          AppointmentDragEndDetails(newAppointment, previousResource, selectedResource, callbackStartDate));
    }
  }
  ...
  void _handleDragStart(DragStartDetails details, bool isNeedDragAndDrop, bool isTimelineView,
      bool isResourceEnabled, double viewHeaderHeight, double timeLabelWidth, double resourceViewSize) {
    if (!CalendarViewHelper.isTimelineView(widget.view)) {
      return;
    }
  ...
  void _updateVisibleDates() {
    ...
    _visibleDates =
        getVisibleDates(currentDate, nonWorkingDays, widget.calendar.firstDayOfWeek, visibleDatesCount)
            .cast();
    _previousViewVisibleDates = getVisibleDates(widget.isRTL ? nextDate : prevDate, nonWorkingDays,
            widget.calendar.firstDayOfWeek, visibleDatesCount)
        .cast();
    _nextViewVisibleDates = getVisibleDates(widget.isRTL ? prevDate : nextDate, nonWorkingDays,
            widget.calendar.firstDayOfWeek, visibleDatesCount)
        .cast();
  ...
  void _updateNextViewVisibleDates() {
    ...
    List<DateTime> dates =
        getVisibleDates(currentViewDate, nonWorkingDays, widget.calendar.firstDayOfWeek, visibleDatesCount)
            .cast();
  ...
  void getRecurrenceRegions(CalendarTimeRegion region, List<CalendarTimeRegion> regions,
      DateTime visibleStartDate, DateTime visibleEndDate, String? calendarTimeZone) {
  ...
  CalendarTimeRegion cloneRecurrenceRegion(
    ...
    final int minutes =
        AppointmentHelper.getDifference(region.actualStartTime, region.actualEndTime).inMinutes;
  ...
  List<Widget> _addViews() {
    ...
    widget.getCalendarState(_updateCalendarStateDetails);
    final _CalendarView previousView =
        _updateViews(_previousView, _previousViewKey, _previousViewVisibleDates);
  ...
  _CalendarView _updateViews(
      _CalendarView view, GlobalKey<_CalendarViewState> viewKey, List<DateTime> visibleDates) {
  ...
  DateTime _updateSelectedDateForRightArrow(
    ...
    if (!CalendarViewHelper.isTimelineView(widget.view)) {
      ...
      if (widget.view == CalendarView.month &&
        ...
        for (int i = 0;
            i < DateTime.daysPerWeek - widget.calendar.timeSlotViewSettings.nonWorkingDays.length;
            i++) {
  ...
  DateTime? _updateSelectedDateForUpArrow(_CalendarView currentView, _CalendarViewState currentViewState,
    ...
    if (widget.view == CalendarView.month) {
      ...
      if (currentViewState._selectedResourceIndex * resourceItemHeight <
        ...
        double scrollPosition =
            currentViewState._timelineViewVerticalScrollController!.offset - resourceItemHeight;
    ...
    if (event.logicalKey == LogicalKeyboardKey.arrowRight) {
      ...
      do {
        selectedDate = _updateSelectedDateForUpArrow(
            currentView, currentViewState, selectedDate, selectedAppointmentDate);
  ...
  KeyEventResult _updatePageUpAndDown(
      RawKeyEvent event, _CalendarViewState currentViewState, bool isResourceEnabled) {
    ...
    if (event.isShiftPressed) {
      if (event.logicalKey == LogicalKeyboardKey.tab) {
        ...
        if (currentSelectedAppointment != null && selectedAppointment == null) {
          ...
          selectedAppointment =
              isAllDay ? allDayAppointmentCollection[allDayAppointmentCollection.length - 1] : null;
  ...
  KeyEventResult _onKeyDown(FocusNode node, RawKeyEvent event) {
    ...
    result = _updateAppointmentSelection(event, currentVisibleViewState, isResourcesEnabled,
        currentSelectedAppointment, currentAllDayAppointment);
      ...
      final List<CalendarAppointment>? selectedAppointments =
          widget.controller.view == CalendarView.month && selectedAppointment == null
              ...
              : selectedAppointment != null
                  ...
                  : null;
      final CalendarElement tappedElement = _updateCalendarStateDetails.selectedDate != null
    ...
    final int previousResourceIndex =
        isResourcesEnabled ? currentVisibleViewState._selectedResourceIndex : -1;

    if ((currentVisibleViewState._selectionPainter!.selectedDate != null &&
            isDateWithInDateRange(
                currentVisibleViewState.widget.visibleDates[0],
                currentVisibleViewState
                    .widget.visibleDates[currentVisibleViewState.widget.visibleDates.length - 1],
      ...
      if (widget.calendar.onSelectionChanged != null &&
        ...
        CalendarViewHelper.raiseCalendarSelectionChangedCallback(
            ...
            selectedDate,
            isResourcesEnabled
                ? widget.resourceCollection![currentVisibleViewState._selectedResourceIndex]
                : null);
      }
      currentVisibleViewState._selectionPainter!.selectedDate = selectedDate;
...
class _CalendarViewState extends State<_CalendarView> with TickerProviderStateMixin {
  ...
  void initState() {
    ...
    _timeIntervalHeight = _getTimeIntervalHeight(widget.calendar, widget.view, widget.width, widget.height,
        widget.visibleDates.length, _allDayHeight, widget.isMobilePlatform);
    if (widget.view != CalendarView.month) {
    ...
    return MouseRegion(
      ...
      child: Stack(children: <Widget>[
        GestureDetector(
          ...
          child: SizedBox(
              width: widget.width, height: widget.height, child: _addMonthView(_isRTL, widget.locale)),
        ),
        ...
        children: <Widget>[
          GestureDetector(
            ...
            child: SizedBox(
                ...
                width: widget.width,
                child: _addDayView(widget.width, _timeIntervalHeight * _horizontalLinesCount!, _isRTL,
                    widget.locale, isCurrentView)),
          ),
          _getResizeShadowView()
        ],
      ),
  ...
  void _scrollToPosition() {
    SchedulerBinding.instance?.addPostFrameCallback((_) {
      ...
      widget.getCalendarState(_updateCalendarStateDetails);
      final double scrollPosition =
          _getScrollPositionForCurrentDate(_updateCalendarStateDetails.currentDate!);
  ...
  double _getPositionFromDate(DateTime date) {
    final int visibleDatesCount = widget.visibleDates.length;
    _timeIntervalHeight = _getTimeIntervalHeight(widget.calendar, widget.view, widget.width, widget.height,
        visibleDatesCount, _allDayHeight, widget.isMobilePlatform);
    double timeToPosition = 0;
    final bool isTimelineView = CalendarViewHelper.isTimelineView(widget.view);
    if (!isTimelineView) {
      timeToPosition = AppointmentHelper.timeToPosition(widget.calendar, date, _timeIntervalHeight);
        ...
        children: <Widget>[
          Positioned(
            ...
            child: ListView(
              ...
              children: <Widget>[
                _getAllDayLayout(timeLabelWidth, panelHeight, allDayExpanderHeight, isCurrentView)
              ],
            ),
          ),
  ...
  Widget _getAllDayLayout(
      double timeLabelWidth, double panelHeight, double allDayExpanderHeight, bool isCurrentView) {
    final bool isDayView = CalendarViewHelper.isDayView(
        widget.view,
        widget.calendar.timeSlotViewSettings.numberOfDaysInView,
        widget.calendar.timeSlotViewSettings.nonWorkingDays,
  ...
  void _onVerticalStart(DragStartDetails details) {
    ...
    if (!CalendarViewHelper.isTimelineView(widget.view) && widget.view != CalendarView.month) {
      ...
      final double allDayPanelHeight =
          _isExpanded ? _updateCalendarStateDetails.allDayPanelHeight : _allDayHeight;
      ...
      _resizingDetails.value.position.value =
          Offset(appointmentView.appointmentRect!.left, details.localPosition.dy);
    }
    ...
    if (CalendarViewHelper.shouldRaiseAppointmentResizeStartCallback(
      ...
      CalendarViewHelper.raiseAppointmentResizeStartCallback(widget.calendar,
          _getCalendarAppointmentToObject(appointmentView!.appointment, widget.calendar), null);
    }
  }
  ...
  void _onVerticalUpdate(DragUpdateDetails details) {
    ...
    if (!CalendarViewHelper.isTimelineView(widget.view) && widget.view != CalendarView.month) {
      _updateMaximumResizingPosition(isForwardResize, isBackwardResize,
          _resizingDetails.value.appointmentView!, allDayPanelHeight, viewHeaderHeight);
      ...
      _updateAutoScrollDay(
          details, viewHeaderHeight, allDayPanelHeight, isForwardResize, isBackwardResize, yPosition);
    }
  ...
  void _onVerticalEnd(DragEndDetails details) {
    ...
    final double timeIntervalHeight = _getTimeIntervalHeight(widget.calendar, widget.view, widget.width,
        widget.height, widget.visibleDates.length, _allDayHeight, widget.isMobilePlatform);
    ...
    final DateTime resizingTime = _timeFromPosition(appointment.actualStartTime,
        widget.calendar.timeSlotViewSettings, yPosition, null, timeIntervalHeight, false)!;
  ...
  void _onHorizontalStart(DragStartDetails details) {
    ...
    if (!isTimelineView && widget.view != CalendarView.month) {
      ...
      _updateMaximumResizingPosition(
          isForwardResize, isBackwardResize, appointmentView, null, viewHeaderHeight);
    } else if (isTimelineView) {
      yPosition -= viewHeaderHeight + timeLabelWidth;
      xPosition = _scrollController!.offset + details.localPosition.dx;
      if (_isRTL) {
        xPosition = _scrollController!.offset +
            (_scrollController!.position.viewportDimension - details.localPosition.dx);
        xPosition =
            (_scrollController!.position.viewportDimension + _scrollController!.position.maxScrollExtent) -
                xPosition;
      }
      ...
      if (isResourceEnabled) {
        resource = widget.calendar.dataSource!.resources![_getSelectedResourceIndex(
            appointmentView.appointmentRect!.top, viewHeaderHeight, timeLabelWidth)];
      }
    ...
    if (CalendarViewHelper.shouldRaiseAppointmentResizeStartCallback(
        widget.calendar.onAppointmentResizeStart)) {
      CalendarViewHelper.raiseAppointmentResizeStartCallback(widget.calendar,
          _getCalendarAppointmentToObject(appointmentView.appointment, widget.calendar), resource);
    }
  }
  ...
  void _onHorizontalUpdate(DragUpdateDetails details) {
    ...
    if (isTimelineView) {
      ...
      _updateAutoScrollTimeline(details, timeIntervalHeight, isForwardResize, isBackwardResize, xPosition,
          yPosition, timeLabelWidth, isResourceEnabled);
    } else if (widget.view == CalendarView.month) {
      final double viewHeaderHeight =
          CalendarViewHelper.getViewHeaderHeight(widget.calendar.viewHeaderHeight, widget.view);
    ...
    if (CalendarViewHelper.shouldRaiseAppointmentResizeUpdateCallback(
      ...
      CalendarViewHelper.raiseAppointmentResizeUpdateCallback(
          ...
          _getCalendarAppointmentToObject(
              _resizingDetails.value.appointmentView!.appointment, widget.calendar),
          null,
          resizingTime,
          _resizingDetails.value.position.value!);
    }
  ...
  void _onHorizontalEnd(DragEndDetails details) {
    ...
    if (_resizingDetails.value.isAllDayPanel ||
      ...
      final DateTime time = _timeFromPosition(resizingTime, widget.calendar.timeSlotViewSettings, xPosition,
          this, timeIntervalHeight, isTimelineView)!;

      resizingTime = DateTime(
          resizingTime.year, resizingTime.month, resizingTime.day, time.hour, time.minute, time.second);
    }
    ...
    if ((_isRTL && _mouseCursor == SystemMouseCursors.resizeLeft) ||
      ...
      if (isMonthView) {
        updatedEndTime = DateTime(resizingTime.year, resizingTime.month, resizingTime.day,
            updatedEndTime.hour, updatedEndTime.minute);
      } else {
        updatedEndTime = resizingTime;
      }
      ...
      if (isMonthView) {
        updatedStartTime = DateTime(resizingTime.year, resizingTime.month, resizingTime.day,
            updatedStartTime.hour, updatedStartTime.minute);
      } else {
        updatedStartTime = resizingTime;
      }
    }
    ...
    if (CalendarViewHelper.isDraggingAppointmentHasDisabledCell(
      ...
      if (CalendarViewHelper.shouldRaiseAppointmentResizeEndCallback(
          widget.calendar.onAppointmentResizeEnd)) {
        CalendarViewHelper.raiseAppointmentResizeEndCallback(widget.calendar, appointment.data, resource,
    ...
    if ((appointment.recurrenceRule != null && appointment.recurrenceRule!.isNotEmpty) ||
      ...
      if (currentRecurrenceIndex != -1) {
        ...
        final DateTime? nextRecurrence = currentRecurrenceIndex >= recurrenceDates.length - 1
            ? null
  ...
  Future<void> _updateAutoScrollDay(DragUpdateDetails details, double viewHeaderHeight,
      double allDayPanelHeight, bool isForwardResize, bool isBackwardResize, double? yPosition) async {
    if (_resizingDetails.value.appointmentView == null) {
      return;
    }
      ...
      _autoScrollTimer = Timer(const Duration(milliseconds: 200), () async {
        ...
        if (xPosition != null &&
          ...
          Future<void> _updateScrollPosition() async {
            ...
            _resizingDetails.value.position.value =
                Offset(xPosition! - 0.1, _resizingDetails.value.position.value!.dy);
  ...
  void _updateMaximumResizingPosition(bool isForwardResize, bool isBackwardResize,
      AppointmentView appointmentView, double? allDayPanelHeight, double? viewHeaderHeight) {
    switch (widget.view) {
      case CalendarView.schedule:
        break;
      case CalendarView.day:
      ...
      case CalendarView.workWeek:
        {
          if (_resizingDetails.value.isAllDayPanel) {
            ...
            final double minimumCellWidth =
                ((widget.width - timeLabelWidth) / widget.visibleDates.length) / 2;
            if (isForwardResize) {
              ...
              child: ListView(
                  ...
                  children: <Widget>[
                    Stack(children: <Widget>[
                      RepaintBoundary(
                          child:
                              _CalendarMultiChildContainer(width: width, height: height, children: <Widget>[
                        RepaintBoundary(
  ...
  AppointmentView? _handleTouchOnMonthView(
    ...
    if (yDetails < viewHeaderHeight) {
      ...
      final bool canRaiseLongPress =
          CalendarViewHelper.shouldRaiseCalendarLongPressCallback(widget.calendar.onLongPress) &&
              !isTapCallback;
  ...
  int _getSelectedResourceIndex(double yPosition, double viewHeaderHeight, double timeLabelSize) {
    final int resourceCount =
        widget.calendar.dataSource != null && widget.calendar.dataSource!.resources != null
            ? widget.calendar.dataSource!.resources!.length
            : 0;
    final double resourceItemHeight = CalendarViewHelper.getResourceItemHeight(
        widget.calendar.resourceViewSettings.size,
    ...
    if (yDetails < viewHeaderHeight) {
      ...
      if (canRaiseLongPress || canRaiseTap || canRaiseSelectionChanged) {
        ...
        if (appointmentView == null) {
          ...
          _updatedSelectionChangedCallback(canRaiseSelectionChanged, previousSelectedDate, selectedResource,
    ...
    if (yDetails < viewHeaderHeight) {
      ...
      if (isTappedOnCount && selectedDate != null) {
        final int currentSelectedIndex =
            DateTimeHelper.getVisibleDateIndex(widget.visibleDates, selectedDate);
    ...
    if (yDetails < viewHeaderHeight) {
      ...
      if (appointmentView != null &&
          (yPosition < allDayHeight - kAllDayAppointmentHeight ||
              _updateCalendarStateDetails.allDayPanelHeight <= allDayHeight ||
              appointmentView.position + 1 >= appointmentView.maxPositions)) {
        if (!CalendarViewHelper.isDateTimeWithInDateTimeRange(widget.calendar.minDate,
                widget.calendar.maxDate, appointmentView.appointment!.actualStartTime, timeInterval) ||
            !CalendarViewHelper.isDateTimeWithInDateTimeRange(widget.calendar.minDate,
      ...
      if (appointmentView == null) {
        if (_isRTL) {
          ...
          _drawSelection(
              xDetails - timeLabelWidth, yDetails - viewHeaderHeight - allDayHeight, timeLabelWidth);
        }
  ...
  bool _isEnabledRegion(double y, DateTime? selectedDate, int resourceIndex) {
    ...
    if (CalendarViewHelper.isTimelineView(widget.view)) {
      ...
      if (_isRTL) {
        minutes =
            ((_scrollController!.offset + (_scrollController!.position.viewportDimension - y)) % viewWidth) ~/
                minuteHeight;
      } else {
    ...
    final DateTime date = DateTime(
        selectedDate.year, selectedDate.month, selectedDate.day, 0, minutes + startDuration.inMinutes);
  ...
  double _getTimeIntervalWidth(
      double timeIntervalHeight, CalendarView view, double width, bool isMobilePlatform) {
  ...
  double _getTimeIntervalHeight(SfCalendar calendar, CalendarView view, double width, double height,
      int visibleDatesCount, double allDayHeight, bool isMobilePlatform) {
    ...
    double timeIntervalHeight = isTimelineView
        ? _getTimeIntervalWidth(
            calendar.timeSlotViewSettings.timeIntervalWidth, view, width, isMobilePlatform)
  ...
  /// scrollable width
  bool _isValidWidth(
      double screenWidth, SfCalendar calendar, int visibleDatesCount, double horizontalLinesCount) {
  ...
  void _updateHoveringForViewHeader(
      Offset localPosition, double xPosition, double yPosition, double viewHeaderHeight) {
    ...
    final DateTime? hoverDate = _getTappedViewHeaderDate(
        Offset(CalendarViewHelper.isTimelineView(widget.view) ? localPosition.dx : xPosition, yPosition),
        widget.width);
  ...
  void _updatePointerHover(Offset globalPosition) {
    ...
    if (widget.view != CalendarView.month && !isTimelineViews) {
      ...
      final AppointmentView? appointment = _appointmentLayout.getAppointmentViewOnPoint(
    ...
    if (hoverDate == null ||
        (isMonthView &&
            !isDateWithInDateRange(widget.calendar.minDate, widget.calendar.maxDate, hoverDate)) ||
        (!isMonthView &&
            !CalendarViewHelper.isDateTimeWithInDateTimeRange(
  ...
  DateTime? _getDateFromPositionForTimeline(double cellWidth, double cellHeight, double x, double y) {
    ...
    if (_isRTL) {
      ...
      rowIndex = (((_scrollController!.offset % _getSingleViewWidthForTimeLineView(this)) + x) / cellWidth)
          .truncate();
    }
    ...
    final double time =
        ((CalendarViewHelper.getTimeInterval(widget.calendar.timeSlotViewSettings) / 60) * rowIndex) +
            widget.calendar.timeSlotViewSettings.startHour;
    final int hour = time.toInt();
    final int minute = ((time - hour) * 60).round();
    if (columnIndex < 0) {
  ...
  void _drawSelection(double x, double y, double timeLabelWidth) {
    ...
    if (selectedDate == null ||
        (isMonthView &&
            !isDateWithInDateRange(widget.calendar.minDate, widget.calendar.maxDate, selectedDate)) ||
        (!isMonthView &&
            !CalendarViewHelper.isDateTimeWithInDateTimeRange(
                widget.calendar.minDate, widget.calendar.maxDate, selectedDate, timeInterval))) {
      return;
...
class _ViewHeaderViewPainter extends CustomPainter {
  ...
  void _addViewHeaderForMonthView(Canvas canvas, Size size, TextStyle viewHeaderDayStyle, double width,
      DateTime today, double weekNumberPanelWidth) {
  ...
  void _addViewHeaderForTimeSlotViews(Canvas canvas, Size size, TextStyle viewHeaderDayStyle,
    ...
    for (int i = 0; i < visibleDatesLength; i++) {
      ...
      _dateTextPainter.paint(
          canvas,
          Offset(
              xPosition + dateXPosition, yPosition + topPadding + _dayTextPainter.height + inBetweenPadding));
      if (!isDayView &&
          showWeekNumber &&
        ...
        _dayTextPainter.paint(
            canvas, Offset(xPosition + (timeLabelWidth / 2 - _dayTextPainter.width / 2), yPosition));
      }
  ...
  void _addMouseHoverForMonth(Canvas canvas, Size size, double xPosition, double yPosition, double width) {
    if (xPosition + (width / 2 - _dayTextPainter.width / 2) <= viewHeaderNotifier.value!.dx &&
        xPosition + (width / 2 - _dayTextPainter.width / 2) + _dayTextPainter.width >=
            viewHeaderNotifier.value!.dx &&
        yPosition - 5 <= viewHeaderNotifier.value!.dy &&
        (yPosition + size.height) - 5 >= viewHeaderNotifier.value!.dy) {
      _drawTodayCircle(
          canvas, xPosition + (width / 2 - _dayTextPainter.width / 2), yPosition, _dayTextPainter,
          hoveringColor: (calendarTheme.brightness == Brightness.dark ? Colors.white : Colors.black87)
              .withOpacity(0.04));
    }
  }

  void _addMouseHoverForTimeSlotView(Canvas canvas, Size size, double xPosition, double yPosition,
      double dateXPosition, double topPadding, bool isToday, int padding) {
    if (xPosition + dateXPosition <= viewHeaderNotifier.value!.dx &&
      ...
      _drawTodayCircle(canvas, xPosition + dateXPosition,
          yPosition + topPadding + _dayTextPainter.height + padding, _dateTextPainter,
...
class _SelectionPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final bool isResourceEnabled =
        isTimeline && CalendarViewHelper.isResourceEnabled(calendar.dataSource, view);
  ...
  DateTime _updateSelectedDate() {
    ...
    final int selectedMinutes =
        ((selectedDate!.hour - startHour) * 60) + (selectedDate!.minute - startMinute.toInt());
      ...
      for (int i = 0; i < visibleDates.length; i++) {
        if (isSameDate(selectedDate, visibleDates[i])) {
          ...
          _xPosition = (i * singleViewWidth) +
              AppointmentHelper.timeToPosition(calendar, selectedDate!, timeIntervalHeight);
          if (isRTL) {
...
class _TimeRulerView extends CustomPainter {
  ...
  /// calendar.
  void _drawTimeLabels(Canvas canvas, Size size, DateTime date, double hour, double xPosition,
    ...
    for (int i = isTimelineView ? 0 : 1;
      ...
      date =
          DateTime(date.year, date.month, date.day, timeSlotViewSettings.startHour.toInt(), minute.toInt());
...
class _CurrentTimeIndicator extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    if (isTimelineView) {
      ...
      canvas.drawLine(
          Offset(viewStartPosition, startYPosition), Offset(viewEndPosition, startYPosition), painter);
    }
  }
...
DateTime? _timeFromPosition(DateTime date, TimeSlotViewSettings timeSlotViewSettings, double positionY,
  ...
  if (isTimelineView) {
    if (currentState!._isRTL) {
      positionY =
          (currentState._scrollController!.offset % _getSingleViewWidthForTimeLineView(currentState)) +
...
class _ResizingAppointmentPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final bool isHorizontalResize =
        resizingDetails.value.isAllDayPanel || isTimelineView || view == CalendarView.month;
    ...
    final bool isRecurrenceAppointment =
        resizingDetails.value.appointmentView!.appointment!.recurrenceRule != null &&
    ...
    if (view != CalendarView.month) {
      _addSubjectTextForTimeslotViews(
          canvas, textStartPadding, xPosition, yPosition, isRecurrenceAppointment, rect);
    } else {
      _addSubjectTextForMonthView(canvas, resizingDetails.value.appointmentView!.appointmentRect!,
          appointmentTextStyle, span, isRecurrenceAppointment, xPosition, rect, yPosition);
    }
  ...
  void _addSubjectTextForTimeslotViews(Canvas canvas, int textStartPadding, double xPosition,
      double yPosition, bool isRecurrenceAppointment, Rect rect) {
    final double totalHeight =
        resizingDetails.value.appointmentView!.appointmentRect!.height - textStartPadding;
    ...
    _textPainter.paint(
        canvas, Offset(xPosition + (isRTL ? 0 : textStartPadding), yPosition + textStartPadding));
  ...
  void _addRecurrenceIcon(
      Rect rect, Canvas canvas, int? textPadding, bool isRecurrenceAppointment, double textSize) {
    ...
    if (view == CalendarView.month) {
      ...
      canvas.drawRRect(
          RRect.fromRectAndRadius(
              Rect.fromLTRB(recurrenceStartPosition, yPosition, recurrenceStartPosition + _textPainter.width,
                  rect.bottom),
...
dynamic _getCalendarAppointmentToObject(CalendarAppointment? calendarAppointment, SfCalendar calendar) {
  ...
  final dynamic customObject =
      calendar.dataSource!.convertAppointmentToObject(calendarAppointment.data, appointment);
