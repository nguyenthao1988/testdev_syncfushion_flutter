...
class SfDataPager extends StatefulWidget {
  ...
  const SfDataPager(
      ...
      : assert(pageCount > 0),
        ...
        assert(
            (firstPageItemVisible || lastPageItemVisible || nextPageItemVisible || previousPageItemVisible) &&
  ...
  void debugFillProperties(DiagnosticPropertiesBuilder properties) {
    ...
    properties
        .add(ObjectFlagProperty<DataPagerDelegate>('delegate', delegate, showName: true, ifNull: 'null'));
    properties.add(
        ObjectFlagProperty<DataPagerController>('controller', controller, showName: true, ifNull: 'null'));
    // ignore: strict_raw_type
    properties.add(ObjectFlagProperty<DataPagerItemBuilderCallback>('pageItemBuilder', pageItemBuilder,
...
class SfDataPagerState extends State<SfDataPager> {
  ...
  void _onInitialDataPagerLoaded() {
    WidgetsBinding.instance?.addPostFrameCallback((_) async {
      ...
      if (widget.initialPageIndex > 0) {
        await _handleDataPagerControlPropertyChanged(property: 'initialPageIndex');
      } else {
  ...
  Future<void> _scrollTo(double offset, {bool canUpdate = false}) async {
    ...
    Future<void>.delayed(Duration.zero, () {
      _scrollController!
          .animateTo(offset, duration: const Duration(milliseconds: 250), curve: Curves.fastOutSlowIn);
    });
  }
  ...
  double _getDataPagerWidth({bool canEnableDataPagerLable = false, bool isDropDown = false}) {
    if (widget.direction == Axis.horizontal) {
  ...
  void _ensureItems() {
    ...
    final int startIndex = _scrollController!.offset <= _scrollController!.position.minScrollExtent
        ? 0
        : _scrollController!.offset ~/ buttonSize;
    final int endIndex = _scrollController!.offset >= _scrollController!.position.maxScrollExtent
        ? _lastPageIndex
  ...
  void _arrangeScrollableItems() {
    _itemGenerator._items.sort((_ScrollableDataPagerItem first, _ScrollableDataPagerItem second) =>
  ...
  Widget _buildDataPagerItem(
    ...
    void _setBorder() {
      border = _dataPagerThemeHelper!.itemBorderWidth != null && _dataPagerThemeHelper!.itemBorderWidth! > 0.0
          ? Border.all(
              width: _dataPagerThemeHelper!.itemBorderWidth!, color: _dataPagerThemeHelper!.itemBorderColor)
        ...
        child: CustomPaint(
          ...
          painter: _DataPagerItemBoxPainter(
              decoration: BoxDecoration(
                  color: itemColor, border: border, borderRadius: _dataPagerThemeHelper!.itemBorderRadius),
  ...
  Widget? _buildHeader() {
    ...
    _headerExtent = children.isEmpty
        ? 0.0
        : children.length * _getButtonSize(widget.navigationItemHeight, widget.navigationItemWidth);
  ...
  Widget _buildBody(BoxConstraints constraint) {
    ...
    final double visibleItemsSize =
        size - (_headerExtent + _footerExtent + widget.visibleItemsCount * buttonSize);
    ...
    _scrollViewPortSize =
        visibleItemsSize <= 0 ? size - _headerExtent - _footerExtent : widget.visibleItemsCount * buttonSize;
  ...
  void _resetScrollOffset(double previousScrollViewPortSize, int oldPageCount) {
    if (_isPregenerateItems &&
        ...
        ((_isOrientationChanged && _scrollViewPortSize > previousScrollViewPortSize) ||
  ...
  Widget _buildDataPagerLabel() {
    ...
    final Widget dataPagerLabel = SizedBox(
        ...
        child: Align(
            alignment: _isRTL ? Alignment.centerLeft : Alignment.centerRight, child: Center(child: label)));
  ...
  void _buildDataPagerWithLabel(BoxConstraints constraint, List<Widget> children) {
    ...
    final Widget dataPager = SizedBox(
        ...
        child: Align(
            ...
            child: SizedBox(
                width:
                    _getDataPagerWidth(canEnableDataPagerLable: canEnablePagerLabel, isDropDown: isDropDown),
  ...
  void didChangeDependencies() {
    ...
    _dataPagerThemeHelper =
        DataPagerThemeHelper(_dataPagerThemeData, Theme.of(context).colorScheme, themeData);
    _isDesktop = kIsWeb ||
  ...
  void ensureItems(int startIndex, int endIndex) {
    ...
    for (int index = startIndex; index <= endIndex; index++) {
      ...
      if (scrollableElement == null) {
        final _ScrollableDataPagerItem? reUseScrollableElement = _items.firstWhereOrNull(
            (_ScrollableDataPagerItem element) => element.index == -1 || !element.isEnsured);
...
class _DataPagerItemRenderObject extends SingleChildRenderObjectWidget {
  _DataPagerItemRenderObject(
      {required Key? key, required this.isDirty, required this.element, required this.child})
...
class _DataPagerItemPanelRenderObject extends MultiChildRenderObjectWidget {
  _DataPagerItemPanelRenderObject(
      {Key? key, required this.isDirty, required this.viewSize, required this.children})
      : super(key: key, children: RepaintBoundary.wrapAll(List<Widget>.from(children)));
...
class _DataPagerItemPanelRenderBox extends RenderBox
    with
        ...
        RenderBoxContainerDefaultsMixin<RenderBox, _DataPagerItemPanelParentData>,
    ...
    while (child != null) {
      ...
      if (child is _DataPagerItemRenderBox) {
        if (child.element.visible) {
          ...
          child.layout(BoxConstraints.tightFor(width: childRect.width, height: childRect.height),
              parentUsesSize: true);
        } else {
...
class DataPagerThemeHelper {
  /// To Do
  DataPagerThemeHelper(
      SfDataPagerThemeData? sfDataPagerThemeData, ColorScheme? colorScheme, ThemeData themeData) {
    ...
    selectedItemTextStyle = sfDataPagerThemeData.selectedItemTextStyle ??
        TextStyle(
            fontFamily: 'Roboto', fontWeight: FontWeight.w400, fontSize: 14, color: colorScheme!.onPrimary);
    ...
    dropdownButtonBorderColor =
        sfDataPagerThemeData.dropdownButtonBorderColor ?? colorScheme!.onSurface.withOpacity(0.12);
  }
