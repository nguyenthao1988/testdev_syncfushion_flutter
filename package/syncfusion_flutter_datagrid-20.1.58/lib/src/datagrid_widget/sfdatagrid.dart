...
class SfDataGridState extends State<SfDataGrid> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    _container =
        VisualContainerHelper(rowGenerator: _rowGenerator, dataGridStateDetails: _dataGridStateDetails!);
  ...
  void _setUp() {
    ...
    _rowSelectionManager =
        _dataGridConfiguration.rowSelectionManager = widget.selectionManager ?? RowSelectionManager();
  ...
  void _processCellUpdate(RowColumnIndex rowColumnIndex) {
    if (rowColumnIndex != RowColumnIndex(-1, -1)) {
      ...
      final int columnIndex =
          grid_helper.resolveToScrollColumnIndex(_dataGridConfiguration, rowColumnIndex.columnIndex);

      final DataRowBase? dataRow =
          _rowGenerator.items.firstWhereOrNull((DataRowBase dataRow) => dataRow.rowIndex == rowIndex);
  ...
  void _updateSummaryColumns(int columnIndex) {
    ...
    if (_dataGridConfiguration.tableSummaryRows.isNotEmpty) {
      for (final GridTableSummaryRow tableSummaryRow in _dataGridConfiguration.tableSummaryRows) {
        ...
        final DataRowBase? summaryDataRow =
            _rowGenerator.items.firstWhereOrNull((DataRowBase row) => row.tableSummaryRow == tableSummaryRow);
  ...
  void _processUpdateDataSource() {
    ...
    setState(() {
      ...
      if (_dataGridConfiguration.currentCell.isEditing) {
        _dataGridConfiguration.currentCell.onCellSubmit(_dataGridConfiguration, canRefresh: false);
      }
      ...
      if (widget.footer != null) {
        final DataRowBase? footerRow = _rowGenerator.items
            .firstWhereOrNull((DataRowBase row) => row.rowType == RowType.footerRow && row.rowIndex >= 0);
        if (footerRow != null) {
  ...
  void _updateProperties(SfDataGrid oldWidget) {
    ...
    final bool isFrozenColumnPaneChanged = oldWidget.frozenColumnsCount != widget.frozenColumnsCount ||
    ...
    final bool isPullToRefreshPropertiesChanged = oldWidget.allowPullToRefresh != widget.allowPullToRefresh ||
      ...
      if (isTableSummaryRowsChanged) {
        if (oldWidget.tableSummaryRows.isNotEmpty) {
          _container.rowGenerator.items.removeWhere((DataRowBase row) =>
              row.rowType == RowType.tableSummaryRow || row.rowType == RowType.tableSummaryCoveredRow);
        }
  ...
  void _updateSelectionController(SfDataGrid oldWidget,
    ...
    if (isSelectionManagerChanged) {
      _rowSelectionManager =
          _dataGridConfiguration.rowSelectionManager = widget.selectionManager ?? _rowSelectionManager!;
      selection_manager.setStateDetailsInSelectionManagerBase(_rowSelectionManager!, _dataGridStateDetails!);
    }
  ...
  void _updateDecoration() {
    final BorderSide borderSide =
        BorderSide(color: _dataGridConfiguration.dataGridThemeHelper!.currentCellStyle.borderColor);
...
class DataGridController extends DataGridSourceChangeNotifier {
  ...
  void moveCurrentCellTo(RowColumnIndex rowColumnIndex) {
    if (_dataGridStateDetails != null) {
      ...
      if (rowColumnIndex != RowColumnIndex(-1, -1) &&
        ...
        final int columnIndex =
            grid_helper.resolveToGridVisibleColumnIndex(dataGridConfiguration, rowColumnIndex.columnIndex);
    ...
    if (_dataGridStateDetails != null) {
      ...
      if (dataGridConfiguration.textDirection == TextDirection.rtl && columnIndex == -1) {
        horizontalOffset = dataGridConfiguration.container.extentWidth -
                    ...
                    horizontalOffset >
                0
            ? dataGridConfiguration.container.extentWidth - dataGridConfiguration.viewWidth - horizontalOffset
            : 0;
      }
  ...
  Future<void> scrollToRow(double rowIndex,
      {bool canAnimate = false, DataGridScrollPosition position = DataGridScrollPosition.start}) async {
...
void notifyDataGridPropertyChangeListeners(DataGridSource source,
  ...
  source._notifyDataGridPropertyChangeListeners(
      rowColumnIndex: rowColumnIndex, recalculateRowHeight: recalculateRowHeight, propertyName: propertyName);
}
...
void updateCurrentCellIndex(DataGridController controller, RowColumnIndex newCurrentCellIndex) {
...
class DataGridThemeHelper {
  ...
  DataGridThemeHelper(SfDataGridThemeData? dataGridThemeData, ColorScheme? colorScheme) {
    ...
    currentCellStyle = dataGridThemeData.currentCellStyle ??
        DataGridCurrentCellStyle(borderColor: colorScheme!.onSurface.withOpacity(0.26), borderWidth: 1.0);
